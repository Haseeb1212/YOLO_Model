...
def detect_image(Yolo, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    ...
    (thresh, blackAndWhiteImage) = cv2.threshold(original_image, 100, 255, cv2.THRESH_BINARY)
    ker = np.ones((1,1))
    ero = cv2.erode(blackAndWhiteImage,kernel=ker,iterations=1)

    pic = cv2.cvtColor(ero,cv2.COLOR_GRAY2RGB)
    pic = cv2.resize(pic, (int(28*ratio), int(28*ratio)))
    
    blank=np.ones((size,size,3),dtype=np.uint8)*255
    
    x_offset=y_offset=50
    blank[y_offset:y_offset+pic.shape[0], x_offset:x_offset+pic.shape[1]] = pic
    original_image = blank
    # contour,hierarchy = cv2.findContours(original_image,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    # blank = np.zeros(original_image.shape,contour)
